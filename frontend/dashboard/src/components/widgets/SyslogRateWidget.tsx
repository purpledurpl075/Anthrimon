...
interface SyslogSummary { total: number; by_severity: Record<string, number>; active_devices: number }

// 7. Syslog rate (messages/hr sparkline)
export function SyslogRateWidget() {
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['syslog-rate'],
    queryFn:  () => api.get<SyslogSummary>('/syslog/summary', { params: { minutes: 1440 } }).then(r => r.data),
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5 h-full">
      ...
      {isLoading ? <p className="text-xs text-slate-400">Loading…</p> : isError ? <p className="text-xs text-red-500">{apiErrorMessage(error)}</p> : !data ? <p className="text-xs text-slate-400">No syslog data</p> : (
